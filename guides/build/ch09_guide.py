...
quiz(doc, [
    ('A report that ends in a recommendation is:', ['Informational','Analytical — and the analytical rules govern','A transcript','Informal and therefore unstructured']),
    ('The pipeline’s critical third step is:', ['Formatting','Concluding — before structuring','Choosing fonts','Scheduling the meeting']),
    ('The top of the pyramid must be:', ['A topic, like “Vendor Analysis”','An answer, like “Renew with TechServe”','A greeting','The method']),
    ('MECE reasons are:', ['As many as possible','Mutually exclusive and collectively exhaustive — no overlaps, no gaps','Chronological','All financial']),
    ('A progress report opens with:', ['Background','The status line — the verdict, no suspense','The team roster','Last month’s recap']),
    ('The “honest yellow” exists because:', ['Dashboards need color','Risks reported while cheap prevent green-to-red surprises — and the audit is retrospective','Yellow is calming','Committees require it']),
    ('Decision minutes record:', ['Who said what, in order','Decisions, owners, deadlines, open questions — same day','Only action items','The debate verbatim']),
    ('In incident reports, active voice matters because:', ['It is shorter','Organizations can only fix what the grammar admits happened','Passive is ungrammatical','Auditors prefer style']),
    ('“Technician error” as a root cause is usually:', ['The end of the investigation','The convenient stop, one “why?” short of the system cause','Legally required','The prevention sentence']),
    ('Choose a table over a chart when readers need:', ['The trend','Exact values — lookup, specifics, audit','Decoration','Proportions']),
    ('An exhibit’s title should:', ['Name the topic: “Response Time Data”','State the takeaway: “Response times fell 41%”','Be omitted','List the sources']),
    ('A truncated bar-chart axis is:', ['Standard practice','The most common honest-numbers-dishonest-picture trick — start at zero or announce loudly','Required for small differences','Tufte-approved']),
    ('The trip report that shaped the roadmap was organized by:', ['Chronology','Decision-relevance — findings that matter, with actions attached','Session titles','Expense category']),
    ('Answer-first documents are cheaper to WRITE because:', ['They are shorter','Selection does the work — deciding what matters replaces narrating everything','Templates exist','AI writes them']),
